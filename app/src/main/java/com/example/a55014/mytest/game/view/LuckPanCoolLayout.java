...
/**
 * @author yy
 * Create by 2019/12/23 19:45
 * to do 大转盘
 */
public class LuckPanCoolLayout extends RelativeLayout {
    ...
    public LuckPanCoolLayout(Context context) {
        this(context, null);
    }

    public LuckPanCoolLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LuckPanCoolLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        MinValue -= DeviceUtils.dip2px(context, 10) * 2;
        LogUtil.d("screenWidth = " + screenWidth + "screenHeight = " + screeHeight + "MinValue = " + MinValue);
    ...
    private void drawSmallCircle(boolean FirstYellow) {
        int pointDistance = radius - DeviceUtils.dip2px(context, 10);
        for (int i = 0; i <= 360; i += 20) {
            int x = (int) (pointDistance * Math.sin(DeviceUtils.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(DeviceUtils.change(i))) + CircleY;

            if (FirstYellow)
                canvas.drawCircle(x, y, DeviceUtils.dip2px(context, 4), yellowPaint);
            else
                canvas.drawCircle(x, y, DeviceUtils.dip2px(context, 4), whitePaint);
