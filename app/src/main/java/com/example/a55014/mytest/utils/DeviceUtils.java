...
public class DeviceUtils {
     ...
     * 这个方法只用于4.4到5.0
     * <p>
     * 注：使用需在styles 中设置<item name="android:windowTranslucentStatus">true</item>
     *
     * @param activity
     * @param topView
     * @param styleColor
     */
    public static void setStatusBarColor(@NonNull Activity activity, @NonNull View topView, @ColorRes int styleColor) {

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
                && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            if (topView != null) {
                ViewGroup.LayoutParams layoutParams = topView.getLayoutParams();
                layoutParams.height = getStatuBarHeight(activity);
